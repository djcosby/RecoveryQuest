        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50 scroll-smooth">
          <div className="max-w-2xl mx-auto bg-white p-6 md:p-10 rounded-xl shadow-sm min-h-full">
            <div className="flex justify-between items-start mb-6">
               <h3 className="text-2xl md:text-3xl font-serif font-bold text-slate-800">{chapter.title}</h3>
               {chapter.quizCompleted && (
                   <div className="flex flex-col items-end">
                       <span className="bg-emerald-100 text-emerald-700 text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter flex items-center">
                           <CheckCircle size={10} className="mr-1" /> Quiz Mastered
                       </span>
                       {chapter.lastScore !== undefined && (
                           <span className="text-[9px] text-slate-400 font-bold mt-1">Score: {chapter.lastScore}/3</span>
                       )}
                   </div>
               )}
            </div>
        ...
        <div className="w-80 bg-white border-l border-slate-200 flex flex-col shrink-0 hidden lg:flex">
          ...
          <div className="p-4 border-t border-slate-200">
            <button 
                ...
                className={`w-full hover:opacity-90 disabled:opacity-50 text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2 transition-all shadow-lg active:scale-95
                ${chapter.quizCompleted ? 'bg-emerald-600 shadow-emerald-200' : 'bg-indigo-600 shadow-indigo-200'}`}
            >
                {isAnalyzing ? <Loader2 className="animate-spin" size={18} /> : (chapter.quizCompleted ? <CheckCircle size={18} /> : <Trophy size={18} />)}
                <span>{chapter.quizCompleted ? 'Retake Quiz' : 'Generate Quiz (+50 XP)'}</span>
            </button>
          </div>
        </div>
      </div>
