...
import { buildDeck, ThoughtCard } from "./filterFeed/filterFeedDeck";
...
export const FilterFeed: React.FC<GameProps> = ({
  ...
  const [deck, setDeck] = useState<ThoughtCard[]>(() =>
    buildDeck({ unitTag, difficulty, specialization, size: 45 })
  );
  ...
  function nextCard() {
    ...
    if (next >= deck.length - 3) {
      setDeck(buildDeck({ unitTag, difficulty, specialization, size: 45 }));
      setIndex(0);
      cardShownAtRef.current = now();
    }
  }
