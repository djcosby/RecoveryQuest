...
import { Settings, Activity, Gauge, Briefcase, Stethoscope, Shield, CheckCircle, Plus, Calendar, Brain, User, Send, ChevronDown, ChevronUp, FileText, BarChart3, Users, ExternalLink, LineChart, ClipboardList, Target, AlertCircle, ArrowRight, Star, MessageSquare, Microscope, MapPin, Flag, Flame, Zap, Award, Edit3, PlusCircle, Loader2, ThumbsUp, Share2, Video, AlertTriangle, Search } from 'lucide-react';
import { PersonalityProfile, Message, Provider, Appointment, WellnessScores, WellnessDimension, CaseFile, Peer, BaselineData, Achievement } from '../../types';
...
import { HealthMetricsDashboard } from '../dashboard/HealthMetricsDashboard';
import { HedisDashboard } from '../dashboard/HedisDashboard';
...
export const ProfileTab: React.FC<{ 
    userLevel: number; 
    userXP: number; 
    initialSubTab?: 'feed' | 'stats' | 'assessments' | 'practice' | 'ai_sponsor' | 'health' | 'network' | 'clinical'; 
    personalityProfile: PersonalityProfile | null; 
    completedQuests: string[]; 
    onStartAssessment: () => void; 
    onCompleteQuest: (xp: number, title: string, questId: string) => void;
    onStartPractice: () => void;
}> = ({ userLevel, userXP, initialSubTab = 'feed', personalityProfile, completedQuests, onStartAssessment, onCompleteQuest, onStartPractice }) => {
  const [subTab, setSubTab] = useState<string>(initialSubTab);
  
  // Chat State
  const [messages, setMessages] = useState<Message[]>([{ id: '1', role: 'model', text: 'Hey Architect. I noticed you logged "High Stress" in your check-in today. Want to run through a box breathing exercise?' }]);
  ...
  const { state: userState, updateWellnessScores, updateCaseFile, updateBaseline, addXP, recordAssessment } = useUserStore();
  const { connections, getPeer, peers } = usePeerStore();
  ...
  const baselineData = userState.baseline;

  const priorityDimension = useMemo(() => {
    if (!wellnessScores) return null;
    let minScore = Infinity;
    let minDim: WellnessDimension | null = null;
    (Object.entries(wellnessScores) as [WellnessDimension, number][]).forEach(([dim, score]) => {
      if (score < minScore) {
        minScore = score;
        minDim = dim;
      }
  ...
  const handleSendMessage = async () => {
    ...
    const userMsg: Message = { id: Date.now().toString(), role: 'user', text: chatInputText };
  ...
  const { average, riskScore } = (() => {
    const total = MOOD_HISTORY.reduce((acc, log) => acc + log.value, 0);
    const count = MOOD_HISTORY.length || 1;
    const average = total / count;
    return { average, riskScore: Math.max(0, 100 - (average / 4) * 100) };
  })();
    ...
    <div className="pb-24 bg-slate-50 min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-4 space-y-6">
              ...
              {/* Statistics Widget (Duolingo Style) */}
              ...
              <div className="bg-white rounded-3xl border-2 border-slate-200 p-5 shadow-sm">
                  ...
                  <div className="space-y-3">
                      ...
                      <button className="w-full flex items-center space-x-3 p-2 hover:bg-slate-50 rounded-xl transition-colors text-left group">
                          ...
                          <span className="font-bold text-slate-600 text-sm group-hover:text-emerald-600">Invite Friends</span>
                          <ChevronDown size={16} className="-rotate-90 ml-auto text-slate-300" />
                      </button>
                  </div>
              </div>
                         ...
                         {userState.caseFile && (
                             <div className="bg-white p-6 rounded-3xl shadow-sm border-2 border-slate-200 grid grid-cols-2 gap-3">
                                ...
                                <div className={`p-3 rounded-xl border-l-4 ${userState.caseFile.recovery.hasSponsor ? 'bg-emerald-50 border-l-emerald-500' : 'bg-amber-50 border-l-amber-500'}`}>
                                 ...
                                 <div className={`p-3 rounded-xl border-l-4 ${userState.caseFile.health.hasPCP ? 'bg-emerald-50 border-l-emerald-500' : 'bg-slate-50 border-l-slate-400'}`}>
